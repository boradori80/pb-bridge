...
const parsePBFile = (text: string, fileName: string): ParsedPB => {
  ...
  if (dwMatch) {
    ...
    //   - = : 등호 매칭
    //   - "(.*?)" | '(.*?)' | [^\s)]+: 큰따옴표 안의 문자열, 작은따옴표 안의 문자열, 또는 따옴표 없는 공백 전까지의 값
  ...
  // 2. DataWindow 컬럼(column) 정의 파싱
  // - 파워빌더 DataWindow는 테이블 필드 정보를 `column(type=char(10) name=id ...)` 과 같이 정의합니다.
  const colRegex = /column\s*\(([\s\S]*?)\)/gi;
  let colMatch;
...
table(column=(type=char(10) updatewhereclause=yes name=id dbname="employee.id" )
 ...
 retrieve="SELECT employee.id, employee.region, employee.rep, employee.sales, employee.status FROM employee WHERE employee.status = ~"Closed~""
)`,
...
// 소스: ${activeFileName} (PB 버전: ${parsed.release})
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 md:p-8 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <section className="lg:col-span-7 flex flex-col gap-6">
            ...
            {/* 3. (DataWindow 전용) 컬럼 목록 및 스키마 정보 */}
            {parsedData.fileType.includes("DataWindow") && parsedData.columns.length > 0 && (
              <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-md shadow-lg flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3.5">
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-400">데이터 컬럼 목록 (Schema Columns)</h4>
                    <p className="text-[10px] text-slate-500 mt-1">DataWindow 내 컬럼 속성 및 원본 DB 스키마 맵</p>
                  </div>
                  {/* 검색 필터 */}
                  <input
                    type="text"
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-950 text-slate-400 font-bold uppercase sticky top-0">
                      <tr className="border-b border-slate-900">
                        <th className="py-2.5 px-3">컬럼 이름</th>
                        <th className="py-2.5 px-3 font-mono">타입 (Type)</th>
                        <th className="py-2.5 px-3 font-mono">DB 바인딩 (dbname)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-900 bg-slate-950/20 font-mono text-slate-300">
                      {filteredColumns.length > 0 ? (
                        filteredColumns.map((col, idx) => (
                          <tr key={idx} className="hover:bg-slate-800/20 transition-all">
                            <td className="py-2 px-3 text-indigo-300 font-semibold">{col.name}</td>
                            <td className="py-2 px-3 text-slate-400">{col.type}</td>
                            <td className="py-2 px-3 text-slate-400 text-[11px]">{col.dbname}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
            ...
            <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-md shadow-lg border-l-4 border-l-indigo-500">
              ...
              <div className="space-y-3.5 text-xs text-slate-300 leading-relaxed font-sans">
                ...
                <div>
                  <span className="inline-block bg-indigo-500/10 text-indigo-300 px-2 py-0.5 rounded text-[10px] font-bold font-mono mb-1">match() / map()</span>
                  <p className="text-[11px] text-slate-400">
                    <code>match()</code>는 파워빌더의 <code>Match()</code>처럼 텍스트 안에서 정규식으로 데이터를 집어오며, <code>map()</code>은 DataStore의 행(Row) 개수만큼 루프(<code>FOR i = 1 TO RowCount()</code>)를 돌아 표/카드를 1:1로 그려내는 작업을 수행합니다.
                  </p>
                </div>
              </div>
            </div>
