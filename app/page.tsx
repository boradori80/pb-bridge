...
export default function PBBridgeDashboard() {
  ...
  // ---------------------------------------------------------------------------------------------------------
  // [교육용 주석 - SQL 포맷터 로직]
  // - SQL 구문의 주요 키워드를 대문자로 변환하고, 줄바꿈을 주어 가독성을 높입니다.
  // ---------------------------------------------------------------------------------------------------------
  const formatSQL = (sql: string): string => {
    ...
    const keywords = [
      ...
      "GROUP BY", "HAVING", "LEFT OUTER JOIN", "RIGHT OUTER JOIN", 
      "INNER JOIN", "JOIN", "UNION"
    ];
  ...
  return (
    <div className="min-h-screen bg-[#070b13] text-slate-100 font-sans flex flex-col antialiased selection:bg-indigo-500 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 md:p-8 flex flex-col gap-6">
        ...
        {/* ========================================================================================= */}
        <section className="bg-[#0b0f19] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
          {/* 상단 탭 및 도구 모음 */}
          <div className="bg-[#121824] px-4 py-2 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3">
            {/* 오라클 토드 스타일 탭 */}
            <div className="flex items-center gap-1">
              <div className="bg-[#0b0f19] border-t-2 border-t-indigo-500 border-x border-slate-800 px-3 py-1.5 rounded-t-lg text-xs font-bold text-indigo-400 flex items-center gap-1.5 cursor-pointer">
                <svg className="w-3.5 h-3.5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" />
                </svg>
                SQL Window (SELECT)
              </div>
              <div className="hover:bg-slate-850 px-3 py-1.5 rounded-t-lg text-xs font-medium text-slate-500 hover:text-slate-300 flex items-center gap-1.5 cursor-pointer transition-colors">
                Explain Plan
              </div>
              <div className="hover:bg-slate-850 px-3 py-1.5 rounded-t-lg text-xs font-medium text-slate-500 hover:text-slate-300 flex items-center gap-1.5 cursor-pointer transition-colors">
                Schema Info
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {isExecutingSql ? "실행 중..." : "Run"}
              </button>

              {/* SQL 포맷팅 토글 버튼 */}
              <button
                onClick={() => setIsSqlFormatted(!isSqlFormatted)}
                disabled={!parsedData.retrieveQuery}
                className={`px-3 py-1 border rounded text-xs font-bold flex items-center gap-1.5 transition-all ${
                  !parsedData.retrieveQuery
              ...
              <button
                ...
                disabled={!parsedData.retrieveQuery}
                className={`px-3 py-1 bg-slate-900 border border-slate-800 hover:bg-slate-800 rounded text-xs font-bold text-slate-400 hover:text-white transition-all flex items-center gap-1.5 ${
          ...
          {/* 에디터 메인 영역 (줄번호 + SQL) */}
          <div className="flex border-b border-slate-800 bg-[#06080f] min-h-[180px] max-h-[350px] overflow-y-auto scrollbar-thin">
            {/* 에디터 좌측 행 번호 */}
            <div className="bg-[#080b13] text-slate-600 text-right pr-3.5 pl-4 py-4 border-r border-slate-850 font-mono text-xs select-none">
            ...
            {/* 에디터 본문 (SQL PRE 태그) */}
            <div className="flex-1 p-4 font-mono text-xs text-indigo-200 overflow-x-auto whitespace-pre-wrap select-text leading-5">
              {/* 
              ...
              */}
              {parsedData.retrieveQuery ? (
                <pre className="m-0 font-mono select-text bg-transparent text-slate-200">
                  <code>{isSqlFormatted ? formatSQL(parsedData.retrieveQuery) : parsedData.retrieveQuery}</code>
                </pre>
              ) : (
          ...
          {/* 쿼리 실행 결과 및 로그 창 */}
          {sqlExecuteLog && (
            <div className="bg-[#05070c] border-b border-slate-800 p-3.5 font-mono text-[11px] text-slate-400 leading-relaxed whitespace-pre-wrap select-text animate-in fade-in duration-200">
              {sqlExecuteLog}
            </div>
          )}

          {/* 하단 상태바 */}
          <div className="bg-[#121824] px-4 py-1.5 flex items-center justify-between text-[10px] font-mono text-slate-500 border-t border-slate-850">
            <div className="flex items-center gap-3">
