...
export default function PBBridgeDashboard() {
  ...
  const renderDynamicInputField = (column: ColumnInfo) => {
    ...
    // 2. 숫자형 (number, long, decimal, int, float, double 등) 탐지
    if (
  ...
  const getSampleValueForColumn = (column: ColumnInfo, rowIndex: number): string => {
    ...
    // 숫자 타입 탐지 (integer, number, decimal, float, double, numeric 등)
    if (
      type.includes("number") ||
      type.includes("decimal") ||
          ...
          <div className="bg-slate-900/30 px-5 py-4 border-b border-slate-900 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-mono font-semibold uppercase px-2 py-1 rounded bg-slate-900 border border-slate-800 text-slate-400">
                Columns: {parsedData.columns.length}ea
              </span>
              <span className="text-[10px] font-mono font-semibold uppercase px-2 py-1 rounded bg-slate-900 border border-slate-800 text-slate-400">
                Rows: 3 (Dummy)
              </span>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-900/60 text-slate-400 font-bold uppercase sticky top-0">
                  <tr className="border-b border-slate-900">
                    ...
                    {/* 
                      [교육용 주석 - 데이터(columns) 개수에 따른 테이블 헤더 동적 생성 원리]
                      -------------------------------------------------------------------------------------
                      웹 초보자를 위한 핵심 매커니즘 설명:
                      
                      1. **동적 HTML 테이블 헤더 생성 원리**:
                         - 일반적인 HTML로 표를 만들 때는 <thead> <tr> 안에 <th>이름</th><th>나이</th> 처럼 컬럼 개수만큼 코드를 고정(Hard-coding)하여 작성합니다.
                         - 하지만 파워빌더 .srd 마다 정의된 컬럼의 종류와 개수가 매번 달라지므로, 고정된 코드로 작성할 경우 유연하게 대응할 수 없습니다.
                         - 이를 해결하기 위해 React(자바스크립트)에서는 파싱되어 들어온 `parsedData.columns` 배열 뒤에 `.map()` 함수를 연계합니다.
                         - `columns.map((column) => <th key={...}>{column.name}</th>)` 코드를 실행하면, React는 배열의 크기(예: 5개 컬럼)를
                           자동으로 측정하여 브라우저가 화면을 그리는 시점에 5개의 `<th>` 요소를 실시간으로 생성하여 화면에 주입합니다.
                         - 이로 인해 어떠한 srd 파일이 들어오더라도 컬럼 수에 완벽히 동기화되어 표의 가로 너비와 헤더가 동적으로 조절됩니다.
                      
                      2. **2차원 그리드 데이터 매핑의 흐름**:
                         - 표(Table)는 가로(Row)와 세로(Column)를 가진 2차원 매트릭스 형태의 데이터 구조입니다.
                         - 이를 그리기 위해 React에서는 2중 루프와 비슷한 형태로 구조화합니다:
                           - **바깥쪽 루프 (`[0, 1, 2].map(rowIndex => ...)`)**: 고정된 3개의 가상 행(Row)을 생성합니다.
                           - **안쪽 루프 (`parsedData.columns.map(column => ...)`)**: 각 행(Row) 내부에서 파싱된 컬럼 개수만큼 셀(Cell, `<td>`)을 생성합니다.
                         - 결과적으로 `행 개수(3) * 열 개수(N)` 만큼의 `<td>` 요소가 동적으로 교차 생성되며 바둑판 모양의 깨끗한 그리드가 완성됩니다.
                      
                      3. **상태(State) 동기화 (Reactive UI)**:
                         - 사용자가 다른 .srd 파일을 업로드하여 `parsedData` 상태가 변경되면 React가 가상 DOM을 분석하여
                           변경된 부분만 화면에 즉시 재렌더링합니다.
                      -------------------------------------------------------------------------------------
                    */}
                      ...
                      >
                        ...
                        {parsedData.columns.map((col, colIdx) => {
                          ...
                          return (
                            <td
                              key={colIdx}
                              className={`py-3 px-4 text-slate-300 border-r border-slate-900/55 last:border-r-0 ${alignClass}`}
                            >
                              <span className="text-xs group-hover:text-white transition-colors">
                                {getSampleValueForColumn(col, rowIndex)}
                              </span>
                            </td>
                          );
                        })}
